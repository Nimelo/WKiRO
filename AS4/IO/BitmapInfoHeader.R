...
BitmapInfoHeader <- setRefClass("BitmapInfoHeader",
	...
	methods = list(
		...
		write = function(conn) {
			writeBin(size, conn)
			writeBin(width, conn)
			writeBin(height, conn)
			writeBin(planes, conn, size = 2)
			writeBin(bitCount, conn, size = 2)
			writeBin(compression, conn)
			writeBin(sizeImage, conn)
			writeBin(xPelsPermeter, conn)
			writeBin(yPelsPermeter, conn)
			writeBin(colorsUsed, conn)
			writeBin(colorsImportant, conn)
		},
